...
export function renderRetroBoard(project: any, sprintNumber: number | string, items: any[]) {
    const formatCategory = (catItems: any[], emptyMsg: string) =>
        catItems.length === 0 ? [emptyMsg] : catItems.map(item => `• **"${item.content}"** (👍 ${item.upvotes || 0})\n   └─ por ${item.author ? `<@${item.author.discord_id}>` : "Anônimo"}`);

    const embed = buildEmbed({
        title: "🏛️  Retrospectiva da Expedição",
        description: [
            HEADERS.sprint, "", `💡 **Quadro de Retrospectiva da Guilda** — **${project.name}** (Sprint #${sprintNumber})`, "",
            "🟢 **O que funcionou bem? (Quest Victories):**", ...formatCategory(items.filter(i => i.category === "went_well"), "*Nenhum item registrado ainda.*"), "",
            "🔴 **O que podemos melhorar? (Boss Traps):**", ...formatCategory(items.filter(i => i.category === "to_improve"), "*Nenhum item registrado ainda.*"), "",
            "💡 **Ações & Ideias para a Próxima Expedição (Quest Tactics):**", ...formatCategory(items.filter(i => i.category === "action_item"), "*Nenhuma ação registrada ainda.*"), "",
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId("retro_add_went_well_btn").setLabel("Funcionou Bem").setEmoji("🟢").setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId("retro_add_to_improve_btn").setLabel("Pode Melhorar").setEmoji("🔴").setStyle(ButtonStyle.Danger),
        new ButtonBuilder().setCustomId("retro_add_action_item_btn").setLabel("Ação / Ideia").setEmoji("💡").setStyle(ButtonStyle.Primary)
    );
...
export const retrospectiveCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (subcommand === "add_item") {
            ...
            return void await interaction.editReply({ content: successMsg("Item adicionado à retrospectiva com sucesso! 💡 (+15 XP concedidos pela contribuição!)") });
        }

        if (subcommand === "conclude") {
            const result = await planningService.concludeRetro(project.id, sprintId, interaction.options.getString("summary") || undefined);
            ...
            const embed = buildEmbed({
                ...
                description: [
                    HEADERS.victory, "", `A Retrospectiva da Expedição #${sprintNum} foi finalizada!`, "",
                    ...
                    "🎯 **Ações Definidas para o Próximo Sprint:**", ...actionLines, "",
